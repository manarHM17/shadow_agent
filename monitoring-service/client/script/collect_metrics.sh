...
# === Récupération du dernier temps d'insertion USB ou "none" ===
if lsusb >/dev/null 2>&1; then
    # Extract the most recent USB insertion time from dmesg
    USB_INSERT_TIME=$(dmesg | grep -i "usb [0-9]*/[0-9]*-.*new device" | tail -1 | awk '{print $1, $2}' | sed 's/^\[\s*\(.*\)\.\(.*\)\]$/\1 \2/')
    if [ -n "$USB_INSERT_TIME" ]; then
        USB_DATA="\"last_insert_time\": \"$USB_INSERT_TIME\""
    else
        USB_DATA="\"usb_devices\": \"none\""
    fi
else
    USB_DATA="\"usb_devices\": \"none\""
fi
...
find "$LOG_DIR" -name "*.json" -type f -mmin +20 -exec rm -f {} \;